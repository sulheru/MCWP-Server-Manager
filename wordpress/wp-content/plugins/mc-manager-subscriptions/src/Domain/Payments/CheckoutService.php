...
final class OptiGrid_Subscriptions_Checkout_Service
{
    ...
    public function create_order(int $user_id, int $plan_id, string $gateway_id, string $idempotency_key): array
    {
        if ($user_id < 1 || get_userdata($user_id) === false) {
            throw new InvalidArgumentException('El usuario seleccionado no existe.');
        }

        $plan=$this->plans->find($plan_id);
        if ($plan === null || empty($plan['is_active'])) {
            throw new InvalidArgumentException('El plan seleccionado no está disponible.');
        }

        $this->gateways->get_enabled($gateway_id);
        $idempotency_key=sanitize_text_field($idempotency_key);
        if ($idempotency_key === '') {
            throw new InvalidArgumentException('Falta la clave de idempotencia.');
        }
        ...
        if ($order === null) {
            throw new RuntimeException('La orden fue creada, pero no pudo recuperarse.');
        }

        return $order;
    }

    public function process_order(string $public_id, string $scenario): array
    {
        ...
        if ($order['status'] !== 'pending') {
            return [
                ...
                'subscription_id'=>!empty($order['subscription_id'])?(int)$order['subscription_id']:null,
                'entitlement_id'=>null,
        ...
        if ($plan === null) {
            throw new RuntimeException('El plan asociado a la orden ya no existe.');
        }

        $gateway=$this->gateways->get_enabled((string)$order['gateway']);
    ...
    {
        $order=$this->create_order($user_id,$plan_id,$gateway_id,$idempotency_key);
        return $this->process_order((string)$order['public_id'],$scenario);
    }
}
