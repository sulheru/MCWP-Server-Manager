...
final class OptiGrid_Subscriptions_Public_Checkout_Controller
{
    ...
    public function __construct($gateways,$plans,$orders,$subscriptions,$entitlements,$checkout)
    { $this->gateways=$gateways; $this->plans=$plans; $this->orders=$orders; $this->subscriptions=$subscriptions; $this->entitlements=$entitlements; $this->checkout=$checkout; }
    ...
    public function register(): void
    {
        ...
        add_action('wp_ajax_'.self::AJAX_CREATE,[$this,'ajax_create_order']);
        add_action('wp_ajax_'.self::AJAX_STATUS,[$this,'ajax_order_status']);
    }

    public function register_assets(): void
    {
        wp_register_style('optigrid-subscriptions-public',OPTIGRID_SUBSCRIPTIONS_URL.'assets/css/public.css',[],OPTIGRID_SUBSCRIPTIONS_VERSION);
        wp_register_script('optigrid-subscriptions-checkout',OPTIGRID_SUBSCRIPTIONS_URL.'assets/js/public-checkout.js',[],OPTIGRID_SUBSCRIPTIONS_VERSION,true);
    }

    public function render_checkout(): string
    {
        wp_enqueue_style('optigrid-subscriptions-public');
        wp_enqueue_script('optigrid-subscriptions-checkout');


        if (!is_user_logged_in()) { return $this->login_required(); }
        try { $this->plans->ensure_sandbox_plan(); }
        catch (Throwable $e) { return $this->public_error($e->getMessage()); }
    ...
    public function render_my_subscription(): string
    {
        ...
        ob_start(); require OPTIGRID_SUBSCRIPTIONS_DIR.'templates/public/my-subscription.php'; return (string)ob_get_clean();
    }

    public function ajax_create_order(): void
    {
        if (!is_user_logged_in()) { wp_send_json_error(['message'=>'Debes iniciar sesión.'],401); }
        check_ajax_referer('optigrid_public_checkout_ajax','nonce');
        try {
            ...
            $order=$this->checkout->create_order(get_current_user_id(),$plan_id,$gateway_id,$key);
            if ($gateway_id !== 'sandbox') { throw new RuntimeException('La pasarela seleccionada todavía no dispone de portal público.'); }
            $url=OptiGrid_Subscriptions_Sandbox_Portal_Controller::portal_url((string)$order['public_id']);
    ...
    {
        if (!is_user_logged_in()) { wp_send_json_error(['message'=>'Sesión no válida.'],401); }
        check_ajax_referer('optigrid_public_checkout_ajax','nonce');
        ...
        if ($order===null) { wp_send_json_error(['message'=>'Orden no encontrada.'],404); }
        wp_send_json_success([
            'publicId'=>$order['public_id'],
            'status'=>$order['status'],
            'amount'=>$order['amount'],
    ...
    {
        return sprintf('<div class="optigrid-public-notice optigrid-public-notice--info"><p>%s</p><p><a class="button" href="%s">%s</a></p></div>',esc_html__('Debes iniciar sesión para contratar una suscripción.','optigrid-subscriptions'),esc_url(wp_login_url(home_url(wp_unslash($_SERVER['REQUEST_URI']??'/')))),esc_html__('Iniciar sesión','optigrid-subscriptions'));
    }
    private function public_error(string $message): string { return '<div class="optigrid-public-notice optigrid-public-notice--error"><p>'.esc_html($message).'</p></div>'; }
}
